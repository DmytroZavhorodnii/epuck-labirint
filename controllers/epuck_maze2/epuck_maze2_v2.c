/*
 * E-puck maze controller — v2
 * Leader: lewa ręka. Follower: prawa ręka. Anti-oscillation: visit counter.
 */
...
/* Anti-oscillation: jeśli komórka odwiedzona > VISIT_MAX razy, zmień regułę */
#define VISIT_MAX         4
...
int main(void) {
  ...
  /* --- v2: reguła ściany i licznik wizyt --- */
  int wall_rule = 0;   /* 0=lewa ręka (leader), 1=prawa ręka (follower) */
  int visits[10][10];
  ...
  while (wb_robot_step(TIME_STEP) != -1) {
    ...
    while (wb_receiver_get_queue_length(receiver) > 0) {
      ...
      if (strncmp(msg, "ID:", 3) == 0 && !role_decided) {
        ...
        if (sscanf(msg + 3, "%d", &rid) == 1 && rid != my_id) {
          ...
          printf("[%s] -> %s (kolega ID=%d) wall_rule=%d\n",
                 name, my_role == ROLE_LEADER ? "LEADER" : "FOLLOWER", rid, wall_rule);
        }
    ...
    if (!role_decided && step > 60) {
      ...
      role_decided = 1;
      wall_rule = 0;
      printf("[%s] solo -> LEADER wall_rule=0\n", name);
    }
    ...
    int w_right =  pv[PS_R]  > SIDE_THR;

    /* Efektywna reguła: flip gdy komórka odwiedzona za dużo razy */
    int cx_cur = (int)(pos[0] / CELL_SIZE);
    int cy_cur = (int)(pos[1] / CELL_SIZE);
    int eff_rule = wall_rule;
    if (cx_cur >= 0 && cx_cur < 10 && cy_cur >= 0 && cy_cur < 10
        && visits[cx_cur][cy_cur] > VISIT_MAX)
      eff_rule = 1 - wall_rule;
    ...
    if (nav == NAV_TURN_RIGHT) {
      ...
      wb_motor_set_velocity(rm, -TURN_SPEED);
      if (++tc >= TURN_STEPS) {
        /* follower jedzie do przodu po skręcie w prawo */
        nav = (wall_rule == 1) ? NAV_FORWARD_AFTER_RIGHT : NAV_FOLLOW;
        tc = 0; fc = 0;
      }
      continue;
    }
    ...
    if (nav == NAV_FORWARD_AFTER_RIGHT) {
      wb_motor_set_velocity(lm, FWD_SPEED);
      wb_motor_set_velocity(rm, FWD_SPEED);
      if (++fc >= FORWARD_STEPS || w_front) { nav = NAV_FOLLOW; fc = 0; }
      continue;
    ...
    /* NAV_FOLLOW — lewa lub prawa ręka (z anti-oscillation flip) */
    if (eff_rule == 0) {
      /* Lewa ręka */
      if (!w_left) {
